...
class GlacierFullDataset(GlacierDataset):
    ...
    def custom_to_tensor(self, image, target):
        to_tensor = ToTensorFull()
        image, mask = to_tensor(image=image, target=target)
        return image, mask


class GlacierFullDataModule(GlacierDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'test' or stage is None:
            self.glacier_test = GlacierFullDataset(mode='test', augmentation=self.augmentation,
        ...
        if stage == 'fit' or stage is None:
            self.glacier_train = GlacierFullDataset(mode='train', augmentation=self.augmentation,
                                                     parent_dir=self.parent_dir,
                                                     bright=self.bright,
                                                     wrap=self.wrap,
                                                     noise=self.noise,
                                                     rotate=self.rotate,
                                                     flip=self.flip)
            self.glacier_val = GlacierFullDataset(mode='val', augmentation=self.augmentation,
                                                   parent_dir=self.parent_dir,
                                                   bright=self.bright,
                                                   wrap=self.wrap,
                                                   noise=self.noise,
